...
class TorneoGestionTest {
    ...
    // ─────────────────────────────────────────────────────────────────
    // Tests para Finalizar Torneo
    // ─────────────────────────────────────────────────────────────────

    @Test
    void testFinalizarTorneoSinTorneoActivo() {
        // instanciamos una gestion limpia sin crear un torneo
        TorneoGestion gestionVacia = new TorneoGestion();

        //comprobamos que lance la excepcion
        assertThrows(IllegalStateException.class, () -> {
            gestionVacia.finalizarTorneo();
        });
    }

    @Test
    void testFinalizarTorneoExitosoSinPartidos() {
        //creamos un torneo normal
        torneoGestion.crearTorneo("Torneo de Verano", disciplina, formato, fechaInicio);

        // como no se han generado partidos todavia, el contador de pendientes es 0
        // Al finalizar, deberia pasar directo sin caerse
        assertDoesNotThrow(() -> {
            torneoGestion.finalizarTorneo();
        });

        //Verificamos que las referencias de la gestion se limpiaron correctamente (quedaron null)
        assertNull(torneoGestion.getTorneo());
        assertNull(torneoGestion.getJornadas());
    }
}
